...
app.post('/api/events/:id/join', authenticateToken, async (req, res) => {
  try {
    ...
    const now = new Date();
    if (now < new Date(ev.fecha_inicio)) return res.status(400).json({ error: 'El evento aún no ha comenzado' });
...
app.post('/api/events/:id/finalize', authenticateToken, async (req, res) => {
  try {
    ...
    for (let i = 0; i < participants.rows.length; i++) {
      ...
      // Dar XP según posición (top 3 reciben bonus)
  ...
  try {
    ...
    for (const badge of allBadges.rows) {
      ...
      switch (badge.codigo) {
        ...
        case 'precision_100': {
          // Verificar última sesión de 10+ con 100%
...
app.post('/api/shop/buy/:itemId', authenticateToken, async (req, res) => {
  try {
    ...
    // Verificar nivel
    if ((req.user.nivel || 1) < shopItem.nivel_requerido) {
      return res.status(400).json({ error: `Se requiere nivel ${shopItem.nivel_requerido}` });
    }
    ...
    await pool.query(
      'INSERT INTO user_inventory (user_id, item_id) VALUES ($1, $2) ON CONFLICT (user_id, item_id) DO UPDATE SET cantidad = cantidad + 1',
      [req.user.id, itemId]
    );
    await pool.query('INSERT INTO xp_history (user_id, cantidad, fuente, referencia_id) VALUES ($1, $2, $3, $4)',
...
// ============================================================
// ENDPOINTS DE NIVELACIÓN AVANZADA
// ============================================================

// Calcular nivel basado en XP + tiempo + precisión
app.get('/api/nivelacion/calcular', authenticateToken, async (req, res) => {
  try {
    ...
    // Fórmula de nivel: base por XP + bonus por tiempo + bonus por precisión
    ...
    const result = await pool.query(
      `SELECT xh.*, 
              CASE 
                ...
                WHEN xh.fuente = 'mision' THEN 'Misión'
...
// ============================================================
// MOTOR DE RESOLUCIÓN PASO A PASO (MEJORADO)
// ============================================================

function factorizarPolinomio(expr) {
  // Motor local de factorización para expresiones comunes
  const pasos = [];
  
  // Detectar factor común: ax^n + bx^m
  ...
  if (fcMatch) {
    ...
    pasos.push({
      math: `${expr}`,
      expl: `Identificamos el factor común. Ambos términos comparten ${variable}^${expMenor} y MCD(${coef1},${coef2}) = ${mcd}.`
    });
    pasos.push({
      math: `${mcd}${variable}^${expMenor}(${coef1/mcd}${variable}^${exp1-expMenor} + ${coef2/mcd}${variable}^${exp2-expMenor})`,
      expl: `Extraemos ${mcd}${variable}^${expMenor} como factor común.`
    });
    return pasos;
  }
  ...
  if (dcMatch) {
    ...
    pasos.push({
      ...
      expl: `Aplicamos la fórmula: (a + b)(a - b) donde a = ${dcMatch[1]} y b = ${dcMatch[2]}`
    });
    return pasos;
  }
...
function generateStepByStep(problem, topic) {
  ...
  switch(topic) {
    case 'factorizacion': {
      ...
      pasos.push({ math: problem, expl: 'Observamos la expresión algebraica para identificar el método de factorización adecuado.' });
      pasos.push({ math: problem, expl: 'Buscamos factor común entre todos los términos. Si no hay, revisamos si es diferencia de cuadrados o trinomio cuadrado perfecto.' });
      pasos.push({ math: problem, expl: 'Aplicamos el método identificado para descomponer en factores más simples.' });
      pasos.push({ math: problem, expl: 'Verificamos multiplicando los factores obtenidos para confirmar que el resultado es la expresión original.' });
      break;
    }
    case 'ecuaciones': {
      pasos.push({ math: problem, expl: 'Identificamosla ecuación. El objetivo es despejar la variable.' });
      // Detectar ecuación lineal: ax + b = c
      ...
      if (linMatch) {
        ...
        pasos.push({ math: `${a}${v} + ${b} = ${c}`, expl: `Movemos el término constante al otro lado: restamos ${b} en ambos lados.` });
      ...
      if (quadMatch) {
        ...
        pasos.push({ math: problem, expl: `Identificamos ecuación cuadrática con a=${a}, b=${b}, c=${c}.` });
        ...
        if (disc > 0) {
          ...
          pasos.push({ math: `${v} = [${-b} ± √(${disc})] / ${2*a}`, expl: `Aplicamos la fórmula cuadrática: x = [-b ± √(b² - 4ac)] / 2a` });
          pasos.push({ math: `${v}₁ = ${x1}, ${v}₂ = ${x2}`, expl: `Dos soluciones reales: ${v}₁ = ${x1} y ${v}₂ = ${x2}` });
        } else if (disc === 0) {
          pasos.push({ math: `${v} = ${-b / (2 * a)}`, expl: `Una solución real (raíz doble): ${v} = ${-b / (2 * a)}` });
        } else {
          pasos.push({ math: `No hay soluciones reales`, expl: `El discriminante es negativo, no hay soluciones en los números reales.` });
        }
        return pasos;
      }
      break;
    ...
    default: {
      pasos.push({ math: problem, expl: 'Analizamos la expresión matemática proporcionada.' });
...
// Endpoint mejorado de resolución paso a paso
app.post('/api/resolver/paso-a-paso', async (req, res) => {
  try {
    const { expression, topic } = req.body;
    
    if (!expression) {
      return res.status(400).json({ error: 'No se proporcionó expresión' });
    }
    ...
    if (configRes.rows.length > 0 || process.env.ANTHROPIC_API_KEY) {
      const systemPrompt = `Eres un tutor de matemáticas experto del TEC de Costa Rica. 
      Explica la resolución de la expresión paso a paso, tipo WolframAlpha.
      Sé MUY detallado, incluyendo cada transformación algebraica y explicando POR QUÉ funciona.
      Responde ÚNICAMENTE en formato JSON:
      {
        "pasos": [
          {"math": "expresión matemática del paso en LaTeX", "expl": "explicación detallada de este paso"}
        ],
      ...
      try {
        ...
        return res.json({
          pasos: pasosLocales.length > 0 ? pasosLocales : [{math: expression, expl: 'No se pudo generar una resolución detallada. Consulta la Biblioteca de Conocimiento para aprender más sobre este tema.'}],
          fuente: 'basico'
        });
      }
    }
    ...
    console.error('Error en resolución:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.listen(port, () => {
  ...
  console.log(`   - Ejercicios (CRUD, generación IA)`);
  ...
  console.log(`   - Nivelación Avanzada 📊`);
  console.log(`   - Resolución Paso a Paso 🔍`);
