...
function getUserTopicPrefs() {
  return arrayFromPreference(state.user?.preferencias_temas, allTopicIds()).filter(id => TOPIC_NAMES[id]);
}

function hasAllNivelPrefs(ids = getUserNivelPrefs()) {
  const all = allNivelIds();
  return ids.length >= all.length && all.every(id => ids.includes(id));
}

function hasAllTopicPrefs(ids = getUserTopicPrefs()) {
...
function openGlossary(termName) {
  ...
  document.getElementById('glossary-mana').textContent = item['ma&ntilde;a'] || item['maña'] || item.mana || '';
...
async function fetchProfile() {
  try {
    ...
    await checkBadges();
    showView(state.user.rol === 'estudiante' && !state.user.preferencias_inicializadas ? 'profile' : 'home');
    updateUI();
  } catch(e) { logout(); }
}
  ...
  document.getElementById('auth-container').innerHTML = `
    <div class="card" style="max-width:400px;margin:4rem auto;">
      ...
      <form onsubmit="handleRegister(event)">
        ...
        <select id="reg-rol" class="btn btn-outline" style="width:100%;text-align:left;background:var(--color-bg);margin-bottom:.75rem;" onchange="toggleRegisterStudentFields()">
...
function toggleRegisterStudentFields() {
  const role = document.getElementById('reg-rol')?.value || 'estudiante';
  const fields = document.getElementById('reg-study-fields');
  if (fields) fields.style.display = role === 'estudiante' ? 'block' : 'none';
}
    ...
    <div class="profile-grid">
      <div class="card profile-panel">
        ...
        <div class="profile-info-grid">
          ...
          <div><strong>XP:</strong> ${state.xp || 0}</div>
        </div>
      </div>
      <div class="card profile-panel">
        <div class="profile-panel-head">
          <h3><i class="ti ti-school"></i> Años</h3>
          <div class="profile-actions">
            <button class="btn btn-outline btn-xs" onclick="setProfileChecks('nivel', true)">Todos</button>
            <button class="btn btn-outline btn-xs" onclick="setProfileChecks('nivel', false)">Limpiar</button>
          </div>
        </div>
        <div class="profile-check-grid">
          ${NIVELES.map(n => `<label class="profile-check"><input type="checkbox" data-profile-group="nivel" value="${n.id}"${nivelPrefs.has(n.id) ? ' checked' : ''}><span>${n.name}</span></label>`).join('')}
        </div>
      </div>
      <div class="card profile-panel profile-panel-wide">
        <div class="profile-panel-head">
          <h3><i class="ti ti-target-arrow"></i> Temas</h3>
          <div class="profile-actions">
            <button class="btn btn-outline btn-xs" onclick="setProfileChecks('tema', true)">Todos</button>
            <button class="btn btn-outline btn-xs" onclick="setProfileChecks('tema', false)">Limpiar</button>
          </div>
        </div>
        <div class="profile-check-grid topic-profile-grid">
          ${sortTopicsForHierarchy(TOPICS).map(t => `<label class="profile-check"><input type="checkbox" data-profile-group="tema" value="${t.id}"${topicPrefs.has(t.id) ? ' checked' : ''}><span>${t.name}</span></label>`).join('')}
        </div>
      </div>
    </div>
    <div class="profile-savebar">
      <button class="btn btn-primary" onclick="saveStudyProfile()"><i class="ti ti-device-floppy"></i> Guardar Perfil</button>
      <button class="btn btn-outline" onclick="showView('home')">Inicio</button>
    </div>`;
}

function setProfileChecks(group, checked) {
...
async function saveStudyProfile() {
  ...
  const body = {
    ...
    preferencias_niveles: preferenciasNiveles.length ? preferenciasNiveles : allNivelIds(),
    preferencias_temas: preferenciasTemas.length ? preferenciasTemas : allTopicIds()
  };
...
function loadRecentExerciseIds() {
  try {
    const raw = JSON.parse(localStorage.getItem('mm_recent_exercise_ids') || '[]');
    state.recentExerciseIds = Array.isArray(raw) ? raw.map(Number).filter(Number.isInteger) : [];
  } catch(e) {
    state.recentExerciseIds = [];
  }
  return state.recentExerciseIds;
}

function rememberExerciseId(id) {
  const num = Number(id);
  if (!Number.isInteger(num) || num <= 0) return;
  const next = [num, ...loadRecentExerciseIds().filter(x => x !== num)].slice(0, 150);
  state.recentExerciseIds = next;
  localStorage.setItem('mm_recent_exercise_ids', JSON.stringify(next));
}

function mergedIds(...lists) {
  return [...new Set(lists.flat().map(Number).filter(Number.isInteger))];
}
...
function preferredTopicScope(topicIds = []) {
  const prefs = getUserTopicPrefs();
  if (hasAllTopicPrefs(prefs)) return topicIds;
  const scoped = topicIds.filter(id => prefs.includes(id));
  return scoped.length ? scoped : topicIds;
}
...
async function generateNewExercise() {
  try {
    ...
    const excludeIds = mergedIds(state.seenExerciseIds);
    const r = await fetch(`${API}/api/ai/generate-exercise`, {
      ...
      body:JSON.stringify({
        ...
        excludeIds,
        recentIds: loadRecentExerciseIds(),
        nivel,
        niveles: practiceNivelScope(nivel),
        strictExclude: true
      })
    });
...
async function saveExerciseToHistory(correct) {
  try {
    await fetch(`${API}/api/reporte/registrar-ejercicio`, {
      method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${state.token}`},
      body:JSON.stringify({ exercise_id:state.currentExercise?.id, topic_id:state.currentTopic.id, correcto:correct, tiempo_segundos:30, hp_antes:state.hp, hp_despues:state.hp, dificultad:state.currentDiff })
    });
    // Registrar XP
...
function missionNivelScope(mission, kind) {
  ...
  return scoped.length ? scoped : missionLevels;
}

function missionTopicScope(mission, kind) {
  const selectedTopic = localStorage.getItem(`mm_${kind}_topic`) || '';
  const levelScope = missionNivelScope(mission, kind);
  let topics = selectedTopic ? (mission.topics || []).filter(id => id === selectedTopic) : [...(mission.topics || [])];
  topics = preferredTopicScope(topics);
  if (levelScope.length) {
    const levelMatched = topics.filter(id => levelScope.some(level => topicMatchesNivel({ id }, level)));
    if (levelMatched.length) topics = levelMatched;
  }
  return topics.length ? topics : [...(mission.topics || [])];
}

function startMission(missionId) {
  const mission = MISSIONS.find(m => m.id === missionId);
  if (!mission) return;
  const kind = mission.isExam ? 'exam' : 'mission';
  const runtimeTopics = missionTopicScope(mission, kind);
  const runtimeNiveles = missionNivelScope(mission, kind);
  state.missionState = { mission, current:0, correct:0, hp:100, exercises:[], runtimeTopics, runtimeNiveles, topicQueue:shuffleArray(runtimeTopics), startTime: Date.now() };
  _doomDeathPlayed = false;
  _prevHp = 100;
...
function getNextMissionTopic() {
  ...
  if (!state.missionState.topicQueue?.length) {
    state.missionState.topicQueue = shuffleArray(state.missionState.runtimeTopics || mission.topics || []);
  }
  return state.missionState.topicQueue.shift() || state.missionState.runtimeTopics?.[0] || mission.topics[0];
}
...
async function loadMissionExercise(topicId, attemptedTopics = []) {
  try {
    ...
    const missionTopics = state.missionState.runtimeTopics || state.missionState.mission.topics || [topicId];
    const missionNiveles = state.missionState.runtimeNiveles || missionNivelScope(state.missionState.mission, mk);
    ...
    const r = await fetch(`${API}/api/ai/generate-exercise`, {
      ...
      body:JSON.stringify({
        ...
        recentIds: loadRecentExerciseIds(),
        nivel,
        niveles: missionNiveles,
        strictExclude: true
      })
    });
    let data = await r.json();
    if (r.status === 409 && attemptedTopics.length < missionTopics.length) {
...
function checkMissionChoice(choice, btn) {
  ...
  saveMissionExerciseToHistory(isCorrect);
}

async function saveMissionExerciseToHistory(correct) {
  try {
    const topicId = state.currentExercise?.topic_id || state.currentTopic?.id || state.missionState?.runtimeTopics?.[0] || state.missionState?.mission?.topics?.[0];
    await fetch(`${API}/api/reporte/registrar-ejercicio`, {
      method:'POST',
      headers:{'Content-Type':'application/json', Authorization:`Bearer ${state.token}`},
      body:JSON.stringify({
        ...
        tiempo_segundos: 30,
        hp_antes: state.hp,
        hp_despues: state.missionState?.hp,
        dificultad: state.currentDiff
      })
    });
  } catch(e) {}
}
