...
function formatExerciseSource(data) {
  if (!data?.source && !data?.archivo_origen && !data?.exam_year) return '';
  const parts = [];
  if (data.archivo_origen) parts.push(data.archivo_origen);
  ...
  try {
    const r = await fetch(`${API}/api/ai/generate-exercise`, {
      ...
      body:JSON.stringify({
        ...
        excludeIds: state.seenExerciseIds   // ← evitar repetidos
      })
    });
    const data = await r.json();
...
function renderMissions(main) {
  ...
  main.innerHTML = `
    ...
    <p style="color:var(--color-text-secondary);margin-bottom:1rem;">Misiones por materia y nivel. Cada tarjeta adapta los temas que entran en esa ruta.</p>
    ${renderMissionFilters('mission', materia, nivel)}
    <div id="mission-list" class="mission-grid">
      ${renderMissionCards(false, saved, materia, nivel)}
    </div>`;
}
...
function renderExams(main) {
  ...
  main.innerHTML = `
    ...
    <p style="color:var(--color-text-secondary);margin-bottom:1rem;">Pruebas curriculares por tema/nivel y ex&aacute;menes de admisi&oacute;n separados.</p>
    ${renderMissionFilters('exam', materia, nivel)}
    <div id="exam-list" class="mission-grid">
      ${renderMissionCards(true, saved, materia, nivel)}
    </div>`;
}

function renderMissionFilters(kind, materia, nivel) {
  return `
...
function missionMatchesFilters(mission, materia, nivel) {
  const missionMateria = mission.materia || getTopicMateriaId(mission.topics?.[0]);
  if (materia && missionMateria !== materia) return false;
  if (nivel && !(mission.niveles || []).includes(nivel)) return false;
  return true;
}

function renderMissionCards(isExam, saved, materia, nivel) {
  const list = MISSIONS.filter(m => !!m.isExam === isExam && missionMatchesFilters(m, materia, nivel));
  if (!list.length) {
...
function renderMissionCard(mission, done, isExam) {
  const levelBadges = (mission.niveles || []).map(id => `<span class="mission-level-chip">${NIVEL_NAMES[id] || id}</span>`).join('');
  const materia = MATERIA_NAMES[mission.materia] || 'Matem&aacute;tica';
...
function updateMissionFilters(kind) {
  ...
  if (list) list.innerHTML = renderMissionCards(kind === 'exam', saved, materia, nivel);
}
...
async function renderKnowledge(main) {
  main.innerHTML = `
    ...
    <p style="color:var(--color-text-secondary);margin-bottom:1.5rem;">Explicaciones detalladas, ejemplos y ma&ntilde;as organizadas por materia y nivel.</p>
    <div class="filter-bar">
      <span class="select-wrap">
        <select id="knowledge-materia" class="select-control" onchange="renderKnowledgeGrid()">${renderMateriaOptions()}</select>
      </span>
      <span class="select-wrap">
        <select id="knowledge-nivel" class="select-control" onchange="renderKnowledgeGrid()">${renderNivelOptions('', 'Todos los niveles')}</select>
      </span>
    </div>
...
function renderAdmin(main) {
  main.innerHTML = `
    ...
    <div class="filter-bar">
      ...
      <span class="select-wrap"><select id="admin-filter" class="select-control" onchange="loadAdminList()">${renderTopicOptions(TOPICS, '', 'Todos los temas')}</select></span>
...
function refreshAdminTopicOptions() {
  ...
  const topics = getFilteredTopics(materia, nivel);
  select.innerHTML = renderTopicOptions(topics, current, 'Todos los temas');
    ...
    document.getElementById('admin-list').innerHTML = list.map(ex => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:.65rem .75rem;background:var(--color-surface-hover);border-radius:var(--radius-md);margin-bottom:.35rem;gap:.75rem;">
        <div style="flex:1;font-size:.85rem;display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;min-width:0;">
          ...
          ${ex.nivel ? `<span class="badge" style="background:var(--color-surface);font-size:.65rem;">${ex.nivel}</span>` : ''}
          ...
          <label class="fc-field">
            ...
            <span class="select-wrap">
              <select id="fc-topic-select" class="select-control">
                ${renderTopicOptions(TOPICS, state.currentTopic?.id || TOPICS[0]?.id || '', null)}
              </select>
            </span>
          </label>
          <label class="fc-field">
