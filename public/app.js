...
const fcState = {
  ...
  known: 0, unknown: 0, topic: null
};

// Temas que tienen contenido suficiente para flashcards
const FC_ENABLED_TOPICS = [
  'conjuntos','numeros-reales','radicales','polinomios','factorizacion',
  'fracciones-alg','ecuaciones','sistemas-ecuaciones','inecuaciones',
  'plano-cartesiano','exp-log','geometria','trigonometria','calculo',
  'mcm-mcd','porcentajes','razones-proporciones','estadistica',
  'probabilidad','logica','matrices','vectores','sucesiones','geo-analitica',
  'tec-logica','tec-matematica'
];

// ---------- Pantalla de selección ----------
function renderFlashcards(main) {
  const availableTopics = TOPICS.filter(t => FC_ENABLED_TOPICS.includes(t.id));
  ...
  main.innerHTML = `
    <div style="max-width:700px;margin:0 auto;">
      ...
      <div style="text-align:center;margin-bottom:2rem;">
        ...
        <h2 style="font-size:1.6rem;margin-bottom:.25rem;">Flashcards</h2>
        <p style="color:var(--color-text-muted);font-size:.9rem;">Repasa conceptos clave con tarjetas interactivas</p>
      </div>

      <!-- Selector -->
      <div class="card" style="margin-bottom:1.25rem;">
        <h3 style="margin-bottom:1.25rem;font-size:1rem;"><i class="ti ti-books" style="color:var(--color-primary);"></i> &iquest;Qu&eacute; tema quieres repasar?</h3>
        <div style="display:grid;grid-template-columns:1fr auto;gap:.75rem;align-items:center;margin-bottom:1rem;">
          <select id="fc-topic-select" style="
          ...
          <select id="fc-count-select" style="
            background:var(--color-bg);
            color:var(--color-text);
            border:1.5px solid var(--color-border);
            border-radius:var(--radius-md);
          ...
          ">
            ...
            <option value="8" selected>8 tarjetas</option>
            <option value="12">12 tarjetas</option>
          </select>
        </div>
        <button class="btn btn-primary" style="width:100%;padding:.9rem;font-size:1rem;" onclick="startFlashcardSession()">
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem;">
        <div class="card" style="text-align:center;padding:1.25rem .75rem;">
          ...
          <div style="font-size:.8rem;color:var(--color-text-secondary);line-height:1.5;">Toca la tarjeta para voltearla</div>
        </div>
        <div class="card" style="text-align:center;padding:1.25rem .75rem;">
          <div style="font-size:1.75rem;margin-bottom:.5rem;">✅</div>
          <div style="font-size:.8rem;color:var(--color-text-secondary);line-height:1.5;">Eval&uacute;a si lo sab&iacute;as o no</div>
        </div>
        <div class="card" style="text-align:center;padding:1.25rem .75rem;">
          <div style="font-size:1.75rem;margin-bottom:.5rem;">⚡</div>
          <div style="font-size:.8rem;color:var(--color-text-secondary);line-height:1.5;">Gana <strong>+20 XP</strong> por cada acierto</div>
        </div>
      </div>
    </div>`;
}

// ---------- Generar tarjetas usando la IA del backend ----------
async function startFlashcardSession() {
  ...
  const main = document.getElementById('main-content');
  main.innerHTML = `
    <div style="max-width:700px;margin:0 auto;text-align:center;padding:4rem 1rem;">
      <div style="font-size:3rem;margin-bottom:1rem;">🃏</div>
      <h3 style="color:var(--color-primary);margin-bottom:.5rem;">Preparando tarjetas...</h3>
      <p style="color:var(--color-text-muted);font-size:.9rem;">${topic.name}</p>
      <div style="margin-top:2rem;background:var(--color-border);border-radius:99px;height:6px;overflow:hidden;">
        <div style="height:100%;width:60%;background:var(--color-primary);border-radius:99px;animation:fc-pulse 1.4s ease-in-out infinite;"></div>
      </div>
      <style>@keyframes fc-pulse{0%,100%{opacity:.5}50%{opacity:1}}</style>
    </div>`;

  try {
    // Generar flashcards usando el mismo endpoint de IA del backend
    // pasando la teoría del tema como contexto
    const r = await fetch(`${API}/api/ai/generate-exercise`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${state.token}` },
      body: JSON.stringify({
        ...
        mode: 'flashcard',
        count: count,
        context: topic.teoria
      })
    });

    const data = await r.json();

    // El backend puede devolver un array de flashcards directo,
    // o podemos construirlos desde la respuesta de ejercicio
    let cards = [];
    ...
    if (Array.isArray(data)) {
      ...
      // Si devuelve un solo ejercicio, usarlo como primera tarjeta
      // y pedir más iterando
      cards = await generateMultipleFlashcards(topicId, count, topic);
    }
    ...
    if (!cards || cards.length === 0) throw new Error('No se pudieron generar las tarjetas');
    ...
    main.innerHTML = `
      <div style="max-width:600px;margin:0 auto;text-align:center;padding:3rem 1rem;">
        ...
        <h3 style="color:var(--color-warning);margin-bottom:.5rem;">No se pudieron cargar las tarjetas</h3>
        <p style="color:var(--color-text-muted);margin-bottom:1.5rem;font-size:.9rem;">${e.message}</p>
        <button class="btn btn-primary" onclick="showView('flashcards')">Volver</button>
      </div>`;
  }
}

// Genera múltiples flashcards llamando al backend varias veces
async function generateMultipleFlashcards(topicId, count, topic) {
  const cards = [];
  const promises = [];

  for (let i = 0; i < count; i++) {
    promises.push(
      fetch(`${API}/api/ai/generate-exercise`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${state.token}` },
  ...
  results.forEach(data => {
    ...
    cards.push({
      front: data.pregunta || '',
      back: data.opciones?.[0] || '',
      latex_front: data.latex || '',
      latex_back: '',
      theory: data.teoria || data.theory || ''
    });
  });
  ...
  main.innerHTML = `
    <div style="max-width:680px;margin:0 auto;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.25rem;">
        <div>
          <p style="font-size:.75rem;color:var(--color-text-muted);margin-bottom:.1rem;text-transform:uppercase;letter-spacing:.08em;">${fcState.topic.name}</p>
          <p style="font-size:.85rem;font-weight:600;">${current + 1} / ${total}</p>
        </div>
        <div style="display:flex;gap:.75rem;align-items:center;">
          <span style="font-size:.85rem;color:var(--color-success);font-weight:700;">✓ ${fcState.known}</span>
          <span style="font-size:.85rem;color:var(--color-error);font-weight:700;">✗ ${fcState.unknown}</span>
          <button class="btn btn-outline btn-sm" onclick="showView('flashcards')" style="padding:.3rem .6rem;">✕</button>
        </div>
      </div>

      <!-- Progress bar -->
      <div style="background:var(--color-border);border-radius:99px;height:5px;margin-bottom:2rem;overflow:hidden;">
        <div style="height:100%;width:${progressPct}%;background:var(--color-primary);border-radius:99px;transition:width .5s ease;"></div>
      </div>

      <!-- Card flip zone -->
      <div id="fc-scene" onclick="flipFlashcard()" style="
        perspective: 1200px;
        cursor: pointer;
        margin-bottom: 1.5rem;
        user-select: none;
      ">
        <div id="fc-inner" style="
          position: relative;
          min-height: 300px;
        ...
        ">
          ...
          ">
            <div style="
              ...
              margin-bottom: 2rem;
            ">
              <i class="ti ti-help-circle"></i> PREGUNTA
            </div>

            ${card.latex_front ? `
              <div id="fc-latex-front" style="
                background: #0d1117;
                border-radius: 12px;
            ...
            <p style="
              margin-top: 2rem; font-size: .75rem;
          ...
          ">
            ...
            <p style="
              ...
              color: var(--color-text); margin: 0; max-width: 460px;
            ">${card.back}</p>
            ...
            ${card.theory ? `
              ...
              ">
                <p style="font-size: .78rem; color: var(--color-text-secondary); line-height: 1.6; margin: 0;">
                  <strong style="color:var(--color-primary);">📚</strong> ${card.theory}
                </p>
              </div>
            ` : ''}
          </div>
      ...
      <div id="fc-eval" style="display:none;gap:.75rem;">
        <button onclick="fcAnswer(false)" style="
          flex:1; padding:1rem; border-radius:var(--radius-md);
          background:rgba(239,68,68,0.1); border:2px solid var(--color-error);
          color:var(--color-error); font-size:.95rem; font-weight:600;
          cursor:pointer; transition:.2s;
        " onmouseover="this.style.background='rgba(239,68,68,0.2)'" onmouseout="this.style.background='rgba(239,68,68,0.1)'">
          <i class="ti ti-x"></i> No lo sab&iacute;a
        </button>
        <button onclick="fcAnswer(true)" style="
          flex:1; padding:1rem; border-radius:var(--radius-md);
          background:rgba(34,197,94,0.1); border:2px solid var(--color-success);
          color:var(--color-success); font-size:.95rem; font-weight:600;
          cursor:pointer; transition:.2s;
      ...
      <p id="fc-hint" style="text-align:center;font-size:.8rem;color:var(--color-text-muted);margin-top:1rem;">
        <i class="ti ti-info-circle"></i> Eval&uacute;a tu respuesta despu&eacute;s de voltear la tarjeta
      </p>
    </div>`;
...
function flipFlashcard() {
  ...
  const evalDiv = document.getElementById('fc-eval');
  ...
  if (!fcState.flipped) {
    ...
    if (evalDiv) { evalDiv.style.display = 'flex'; }
...
async function fcAnswer(knew) {
  ...
  if (fcState.index >= fcState.cards.length) {
    ...
    if (scene) {
      scene.style.transition = 'opacity .18s,transform .18s';
      scene.style.opacity = '0';
      scene.style.transform = knew ? 'translateX(40px)' : 'translateX(-40px)';
      setTimeout(() => renderFlashcardDeck(main), 180);
    } else {
      renderFlashcardDeck(main);
    }
    updateUI();
...
function renderFlashcardResults(main) {
  ...
  const msg = pct >= 80 ? '¡Excelente dominio!' : pct >= 50 ? '¡Buen avance, sigue así!' : '¡Sigue practicando, tú puedes!';
  const barColor = pct >= 70 ? 'var(--color-success)' : pct >= 40 ? 'var(--color-warning)' : 'var(--color-error)';

  main.innerHTML = `
    <div style="max-width:520px;margin:0 auto;text-align:center;">

      <div style="font-size:4.5rem;margin-bottom:.75rem;">${emoji}</div>
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.75rem;">
        <div class="card" style="padding:1.25rem .5rem;">
          <div style="font-size:2rem;font-weight:800;color:var(--color-success);">${fcState.known}</div>
          <div style="font-size:.72rem;color:var(--color-text-muted);margin-top:.2rem;">Sab&iacute;a</div>
        </div>
        <div class="card" style="padding:1.25rem .5rem;">
          <div style="font-size:2rem;font-weight:800;color:var(--color-error);">${fcState.unknown}</div>
          <div style="font-size:.72rem;color:var(--color-text-muted);margin-top:.2rem;">A repasar</div>
        </div>
        <div class="card" style="padding:1.25rem .5rem;">
          <div style="font-size:2rem;font-weight:800;color:var(--color-warning);">+${fcState.known * 20}</div>
          <div style="font-size:.72rem;color:var(--color-text-muted);margin-top:.2rem;">XP ganada</div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:1.5rem;text-align:left;">
        <div style="display:flex;justify-content:space-between;margin-bottom:.6rem;">
          <span style="font-size:.85rem;font-weight:600;">Dominio del tema</span>
          <span style="font-size:.85rem;font-weight:700;color:${barColor};">${pct}%</span>
        </div>
        <div style="background:var(--color-border);border-radius:99px;height:10px;overflow:hidden;">
          <div style="height:100%;width:${pct}%;background:${barColor};border-radius:99px;transition:width 1.2s ease;"></div>
        </div>
      </div>

      <!-- Acciones -->
      <div style="display:flex;flex-direction:column;gap:.75rem;">
        <button class="btn btn-primary" onclick="showView('flashcards')" style="padding:.9rem;">
