...
function exerciseToFlashcard(ex) {
  const options = parseJsonArray(ex.options);
  const steps = parseJsonArray(ex.solution_steps);
  ...
  return {
    id: ex.id,
    front: ex.question || ex.latex_content || 'Ejercicio de repaso',
    back,
    latex_front: ex.latex_content || '',
    latex_back: firstStep.math || '',
...
app.post('/api/ai/generate-flashcards', authenticateToken, async (req, res) => {
  try {
    ...
    const count = Math.min(25, Math.max(1, parseInt(req.body.count, 10) || 8));
